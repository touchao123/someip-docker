...
#include "schema/fake_spd_acc.pb.h"
    ...
    void notify() {
        ...
        while (running_) {
            ...
            while (is_offered_ && running_) {
                ...
                v2x_demo_msg.set_id(counter++);
                v2x_demo_msg.set_velocity(15+counter);
                v2x_demo_msg.set_acc(1.5+counter);
                v2x_demo_msg.set_timestamp(millisec);
                its_size = v2x_demo_msg.ByteSize();
                v2x_demo_msg.SerializeToArray(its_data,its_size);
               
                // std::cout << "string:"<< v2x_demo_msg.SerializeAsString()<< std::endl;
                // std::cout << "vec.size() "<<vec.size()<<" v2x_demo_msg.ByteSize():" << v2x_demo_msg.ByteSize()<<std::endl;
                {


                    std::lock_guard<std::mutex> its_lock(payload_mutex_);
                    payload_->set_data(its_data, its_size);
                    // payload_->set_data(vec);
                    // its_size = v2x_demo_msg.ByteSize();
